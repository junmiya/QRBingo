...
- [x] T051 [P] リーチ表示(US-6)— 2026-07-18 実装: reportReach Function(サーバー検証・虚偽報告は不登録)+ games/{id}/reaches サブコレクション(host のみ read)+ ホストのリーチ中リスト(人数・本数・時点)+ プレイヤー側のリーチマス点滅/バナーパルス/ビンゴ紙吹雪。ビンゴ成立で自動的にリストから除去。games ドキュメントの reachCount 集計フィールドは未使用のまま(プレイヤー全体向け人数表示を作る際に検討)
